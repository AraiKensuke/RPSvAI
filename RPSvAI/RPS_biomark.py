...
def amplitude_ts1(probs, win):
    L = probs.shape[1]
    amp_ts = _N.empty(L - win)
        
    for t in range(L-win):
        srtd = _N.sort(probs[:, t:t+win], axis=1) # 9 sorted arrays of size win
        #AMP = _N.max(probs[:, t:t+win], axis=1) - _N.min(probs[:, t:t+win], axis=1)
        AMP = _N.mean(srtd[:, win-11:], axis=1) - _N.mean(srtd[:, 0:11], axis=1)
        #  AMP is size 9
        #amp_ts[t] = _N.sum(_N.sort(AMP)[7:])
        amp_ts[t] = _N.max(AMP)
...
def t_near13(probs):
    ...
    for ic in range(9):
        near0[ic] = len(_N.where(probs[ic] < eh*2)[0])
        near13[ic] = len(_N.where((probs[ic] > elow) & (probs[ic] < ehig))[0])
        near1[ic] = len(_N.where(probs[ic] > 1-eh*2)[0])

    return near0, near13, near1
...
if data == "TMB2":
    ...
    partIDs, dats, cnstrs, has_domainQs, has_domainQs_wkeys = _rt.filterRPSdats(data, dates, visits=visits, domainQ=(_rt._TRUE_ONLY_ if look_at_AQ else _rt._TRUE_AND_FALSE_), demographic=_rt._TRUE_AND_FALSE_, mentalState=_rt._TRUE_AND_FALSE_, min_meanIGI=500, max_meanIGI=15000, minIGI=20, maxIGI=30000, MinWinLossRat=0.2, has_useragent=True, has_start_and_end_times=True, has_constructor=True, blocks=1)
...
netwins1  = _N.empty(len(partIDs), dtype=_N.int)
netwins2  = _N.empty(len(partIDs), dtype=_N.int)

beat_mean  = _N.empty(len(partIDs))
mimic_mean = _N.empty(len(partIDs))
...
ccAMPS12   = _N.empty(len(partIDs))
ccAMPS13   = _N.empty(len(partIDs))
ccAMPS14   = _N.empty(len(partIDs))
ccAMPS23   = _N.empty(len(partIDs))
ccAMPS24   = _N.empty(len(partIDs))
ccAMPS34   = _N.empty(len(partIDs))
near13_1   = _N.empty((len(partIDs), 9))
near13_2   = _N.empty((len(partIDs), 9))
near13_3   = _N.empty((len(partIDs), 9))
near13_4   = _N.empty((len(partIDs), 9))
near0_1   = _N.empty((len(partIDs), 9))
near0_2   = _N.empty((len(partIDs), 9))
near0_3   = _N.empty((len(partIDs), 9))
near0_4   = _N.empty((len(partIDs), 9))
near1_1   = _N.empty((len(partIDs), 9))
near1_2   = _N.empty((len(partIDs), 9))
near1_3   = _N.empty((len(partIDs), 9))
near1_4   = _N.empty((len(partIDs), 9))


mst_var_DSUWTL = _N.empty(len(partIDs))
mst_var_DSURPS = _N.empty(len(partIDs))
mst_var_RPSWTL = _N.empty(len(partIDs))
mst_var_DSUAIRPS = _N.empty(len(partIDs))
...
cntrsDSUAIRPS = _N.empty((len(partIDs), 2))
...
RPSRPS_corrs = _N.empty((36, len(partIDs)))
RPSAIRPS_corrs = _N.empty((36, len(partIDs)))
...
sum_sd_RPSRPS = _N.empty((len(partIDs), 3, 3))
sum_sd_RPSAIRPS = _N.empty((len(partIDs), 3, 3))
...
entropyDSUAIRPS_D = _N.empty(len(partIDs))   #  how different are D across WTL conditions
entropyDSUAIRPS_S = _N.empty(len(partIDs))
entropyDSUAIRPS_U = _N.empty(len(partIDs))

entropyRPSAIRPS_R = _N.empty(len(partIDs))
entropyRPSAIRPS_S = _N.empty(len(partIDs))
entropyRPSAIRPS_P = _N.empty(len(partIDs))
entropyRPSRPS_R = _N.empty(len(partIDs))
entropyRPSRPS_S = _N.empty(len(partIDs))
entropyRPSRPS_P = _N.empty(len(partIDs))
...
for partID in partIDs:
    ...
    ccAMPS12[pid-1], pv = _ss.pearsonr(amp1, amp2)
    ccAMPS13[pid-1], pv = _ss.pearsonr(amp1, amp3)
    ccAMPS14[pid-1], pv = _ss.pearsonr(amp1, amp4)    
    ccAMPS23[pid-1], pv = _ss.pearsonr(amp2, amp3)
    ccAMPS24[pid-1], pv = _ss.pearsonr(amp2, amp4)
    ccAMPS34[pid-1], pv = _ss.pearsonr(amp3, amp4)                
    ...
    sum_sd_RPSRPS[pid-1] = sdsRPSRPS
    sum_sd_RPSAIRPS[pid-1] = sdsRPSAIRPS        
    ...
    netwins1[pid-1] = _N.sum(_hnd_dat[0:150, 2])
    netwins2[pid-1] = _N.sum(_hnd_dat[150:, 2])    
...
s_rps0, s_rps1, s_rps2, AIent1, AIent2, AIent3, AIent4, AIent5, AIent6, AIent7, AIent8, mn_diff_top2, sd_diff_top2, mnFt1, mnFt2, mnFt3, mnFt4, mnFt5, mnFt6, mnFt7, mnFt8, sdFt1, sdFt2, sdFt3, sdFt4, sdFt5, sdFt6, sdFt7, sdFt8, aift1, aift2, aift3, aift4, aift5, aift6, aift7, aift8, aift9, aift10, aift11, aift12, aift13, aift14, aift15, aift16, aift17, aift18, aift19, aift20, aift21, aift22, aift23, aift24 = _aift.perceptron_features(all_AI_weights, all_AI_preds, partIDs)
...
cntrsDSUAIRPS /= _N.sum(cntrsDSUAIRPS, axis=1).reshape(len(partIDs), 1)
cntrmvs_DSUAIRPS = cntrsDSUAIRPS[:, 1] - cntrsDSUAIRPS[:, 0]
cntrsRPSWTL /= _N.sum(cntrsRPSWTL, axis=1).reshape(len(partIDs), 1)
cntrmvs_RPSWTL = cntrsRPSWTL[:, 1] - cntrsRPSWTL[:, 0]

mst_var_RPSWTL2 = mst_var_DSUWTL - mst_var_RPSWTL
mst_var_DSURPS2 = mst_var_DSUWTL - mst_var_DSURPS
mst_var_DSUAIRPS2 = mst_var_DSUWTL - mst_var_DSUAIRPS

#cntrmvsDIFF = cntrmvs_DSUWTL - cntrmvs_DSUAIRPS
#cntrmvsDIFF = cntrmvs_DSUWTL - cntrmvs_DSURPS
cntrmvsDIFF = cntrmvs_DSUWTL - cntrmvs_DSURPS
#cntrmvsDIFF3 = cntrmvs_DSUAIRPS - cntrmvs_DSURPS
...
near1 = (near0_1 + near1_1 - _N.abs(near0_1 - near1_1)).reshape((len(partIDs), 3, 3))
near2 = (near0_2 + near1_2 - _N.abs(near0_2 - near1_2)).reshape((len(partIDs), 3, 3))
near4 = (near0_4 + near1_4 - _N.abs(near0_4 - near1_4)).reshape((len(partIDs), 3, 3))

s1 = _N.std(sum_sd_DSUWTL, axis=0).reshape(1, 3, 3)
s2 = _N.std(sum_sd_RPSWTL, axis=0).reshape(1, 3, 3)
s4 = _N.std(sum_sd_DSUAIRPS, axis=0).reshape(1, 3, 3)
n1 = _N.std(near1, axis=0).reshape(1, 3, 3)
n2 = _N.std(near2, axis=0).reshape(1, 3, 3)
n4 = _N.std(near4, axis=0).reshape(1, 3, 3)

scale = 0.3
#sum_sd_DSUWTL = sum_sd_DSUWTL / s1 + scale*(near1 / n1)
#sum_sd_RPSWTL = sum_sd_RPSWTL / s2 + scale*(near2 / n2)
#sum_sd_DSUAIRPS = sum_sd_DSUAIRPS / s4 + scale*(near4 / n4)
feats_FCx = ["sum_sd_DSUWTL", "sum_sd_DSUAIRPS", "sum_sd_RPSWTL",
             "DSUWTL_corrs", "DSUAIRPS_corrs", "RPSWTL_corrs", "RPSRPS_corrs", "RPSAIRPS_corrs", "cntrmvs_DSUWTL", "cntrmvs_DSUAIRPS", "cntrmvs_RPSWTL", "cntrmvs_DSURPS",
#feats_FCx = ["sum_sd_DSUWTL", "sum_sd_DSURPS", "sum_sd_RPSWTL",
#             "DSURPS_corrs", "DSUWTL_corrs", "RPSWTL_corrs", "cntrmvs_DSUWTL", "cntrmvs_DSUAIRPS", "cntrmvs_RPSWTL",
             "cntrmvsDIFF",
             "entropyRPSRPS_R", "entropyRPSRPS_S", "entropyRPSRPS_P",
             "entropyRPSAIRPS_R", "entropyRPSAIRPS_S", "entropyRPSAIRPS_P",
             "entropyDSUWTL_D", "entropyDSUWTL_S", "entropyDSUWTL_U",
             "entropyDSUAIRPS_D", "entropyDSUAIRPS_S", "entropyDSUAIRPS_U", 
             "entropyDSURPS_D", "entropyDSURPS_S", "entropyDSURPS_U", "sd_Mimic", "sd_Beat", "sd_Mimic2", "sd_Beat2", "sd_Lose", "sd_Lose2", "ccAMPS12", "ccAMPS13", "ccAMPS14", "ccAMPS23", "ccAMPS24", "ccAMPS34"]

             #"entropyDSURPS_D", "entropyDSURPS_S", "entropyDSURPS_U", "sd_Mimic", "sd_Beat", "sd_Mimic2", "sd_Beat2", "sd_Lose", "sd_Lose2", "ccAMPS12", "ccAMPS13", "ccAMPS14", "ccAMPS23", "ccAMPS24", "ccAMPS34"]             
#feats_FCx = ["ccAMPS13", "ccAMPS14", "ccAMPS34", "ccAMPS12", "ccAMPS23", "ccAMPS24"]
#feats_FCx = ["ccAMPS13", "ccAMPS14", "ccAMPS34"]
#feats_FCx = []
...
cmp_againsts = feats_FCx + feats_FRx + feats_FOx + feats_FAx + feats_FTx
cmp_againsts_name = []
...
dmpout = open(workdirFN("AQ28_vs_RPS_%(v)d_%(wt)d%(w)d%(s)d.dmp" % {"v" : visit, "wt" : win_type, "w" : win, "s" : smth, "wd" : os.environ["RPSWORKDIR"]}), "wb")
...
for pr in ["12", "13", "14", "23", "24", "34"]:
    print("************   pair   is %s" % pr)
    for star in ["AQ28scrs", "soc_skils", "imag", "rout", "switch", "fact_pat"]:
        exec("tar = %s" % star)
        exec("cc  = ccAMPS%s" % pr)        
        pc, pv = _ss.pearsonr(cc[filtdat], tar[filtdat])
        print("%(tar)s:    %(pc).3f  %(pv).3f" % {"pc" : pc, "pv" : pv, "tar" : star})
    print("")


fig = _plt.figure(figsize=(8, 8))
_plt.suptitle("which component has highest time spent in 1/3 zone")
fig.add_subplot(2, 2, 1)
_plt.hist(_N.argmax(near13_1[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 2)
_plt.hist(_N.argmax(near13_2[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 3)
_plt.hist(_N.argmax(near13_3[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 4)
_plt.hist(_N.argmax(near13_4[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)
_plt.savefig("most_time_spent_near_13_for_cond_prob_comp_%d" % SHF_NUM)



fig = _plt.figure(figsize=(8, 8))
_plt.suptitle("which component has highest time spent near 0")
fig.add_subplot(2, 2, 1)
_plt.hist(_N.argmax(near1_1[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 2)
_plt.hist(_N.argmax(near1_2[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 3)
_plt.hist(_N.argmax(near1_3[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 4)
_plt.hist(_N.argmax(near1_4[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)
_plt.savefig("most_time_spent_near_0_for_cond_prob_comp_%d" % SHF_NUM)

fig = _plt.figure(figsize=(8, 8))
_plt.suptitle("which component has highest time spent near 1")
fig.add_subplot(2, 2, 1)
_plt.hist(_N.argmax(near0_1[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 2)
_plt.hist(_N.argmax(near0_2[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 3)
_plt.hist(_N.argmax(near0_3[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)

fig.add_subplot(2, 2, 4)
_plt.hist(_N.argmax(near0_4[filtdat], axis=1), bins=_N.linspace(-0.5, 8.5, 10), density=True)
_plt.ylim(0, 0.25)
_plt.savefig("most_time_spent_near_1_for_cond_prob_comp_%d" % SHF_NUM)
...
for i1 in range(3):
    for i2 in range(i1+1, 3):
        pcs = _N.empty(len(partIDs))
        for i in range(len(partIDs)):
            y1 = _N.convolve(all_AI_weights[i, 3:, i1, i2, 1], gk, mode="same")
            y2 = _N.convolve(all_AI_weights[i, 3:, i2, i1, 0], gk, mode="same")
            #pc, pv = _ss.pearsonr(all_AI_weights[i, 3:, i1, i2, 0], all_AI_weights[i, 3:, i2, i1, 0])
            pc, pv = _ss.pearsonr(y1, y2)
            pcs[i] = pc
        for star in ["AQ28scrs", "soc_skils", "imag", "rout", "switch", "fact_pat"]:
            exec("tar = %(t)s" % {"t" : star})
            pc, pv = _ss.pearsonr(pcs[filtdat], tar[filtdat])
            print("%(1)d %(2)d   %(pc).3f" % {"1" : i1, "2" : i2, "pc" : pc})
            #fig = _plt.figure()
            #_plt.scatter(pcs[filtdat], tar[filtdat])


# pcs = _N.empty(len(partIDs))
# for i in range(len(partIDs)):
#     pc1, pv1 = _ss.pearsonr(all_AI_weights[i, 10:, 1, 0, 0], 
#                             all_AI_weights[i, 10:, 2, 0, 0])
#     pc2, pv2 = _ss.pearsonr(all_AI_weights[i, 10:, 0, 1, 0], 
#                             all_AI_weights[i, 10:, 0, 2, 0])
#     pcs[i] = pc1 + pc2
#     #print("%(1).3f   %(2).3f   %(3).3f" % {"1" : pc1, "2" : pc2, "3" : pc3})
# for star in ["AQ28scrs", "soc_skils", "imag", "rout", "switch", "fact_pat"]:
#     exec("tar = %s" % star)
#     pc, pv = _ss.pearsonr(pcs[filtdat], tar[filtdat])
#     print(pc)
