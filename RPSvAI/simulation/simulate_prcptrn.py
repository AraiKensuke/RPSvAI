...
def recreate_percep_istate(hnd_dat, ini_percep_str, fin_percep_str):
    ini_as_arr = _N.array(ini_percep_str.split(" "), dtype=_N.float32)
    fin_as_arr = _N.array(fin_percep_str.split(" "), dtype=_N.float32)

    recr_prc_mvs = _N.zeros(hnd_dat.shape[0], dtype=_N.int32)
