...
zipped_dir = "%(datdir)s/%(expt)s" % {"expt" : expt, "datdir" : os.environ["TAISENDATA"]}
...
if expt == "TMB1":
    ...
    fns, dats, cnstrs, has_domainQs, has_domainQs_wkeys = _rt.filterRPSdats(expt, dates, visits=visits, domainQ=(_rt._TRUE_AND_FALSE_), demographic=_rt._TRUE_AND_FALSE_, mentalState=_rt._TRUE_AND_FALSE_, min_meanIGI=300, max_meanIGI=20000, minIGI=10, maxIGI=30000, MinWinLossRat=0., has_useragent=True, has_start_and_end_times=True, has_constructor=True, blocks=1)    
...
outfn = "%(expt)sfns_%(v)s.txt" % {"expt" : expt, "v" : str(visits)}
fp = open(outfn, "w")
